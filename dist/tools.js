...
async function readSingleFile(filePath, offset, limit) {
    ...
    try {
        ...
        if (name === "write") {
            ...
            if (args.content === undefined)
                return { content: "Missing 'content' argument", isError: true };
        ...
        if (name === "edit") {
            if (!args.path || !args.oldText || !args.newText)
                return { content: "Missing 'path', 'oldText', or 'newText' argument", isError: true };
        ...
        if (name === "grep") {
            if (!args.pattern)
                return { content: "Missing 'pattern' argument", isError: true };
            ...
            try {
                ...
                if (err.stdout)
                    return { content: summarizeToolOutput(err.stdout), isError: false };
